...
public class Controller {
    ...
    public static void runLoggedAccountUser(TrazAqui trazAqui){
        ...
        switch(opcao) {
            ...
            String codEnc = trazAqui.getNewCode(TipoConta.Utilizador);
            ...
            if (trazAqui.checkIfEntityWorkedForUser(code1)){
                ...
                Map<String,List<Encomenda>> historico = trazAqui.getHistoricoUser();
                List<Encomenda> list = historico.get(code1).stream()
                                                         .filter(a->(a.getData().isBefore(fim) && a.getData().isAfter(inicio)))
                                                         .collect(Collectors.toList());
                Collections.sort(list, (x, y) -> x.getData().compareTo(y.getData()));                                             
            Menu.printHistorico(list,code1,inicio,fim);
            }
        }
            else Menu.errors(4);
            break;
